...
for(exp_num in 1:10){
  acc = c(
  TAN_CV(IEM_austral,5),
  TAN_CV(IEM_breast,5),
  TAN_ST(IEM_chess, 1066),
  TAN_CV(IEM_cleve,5),
  TAN_CV(IEM_corral,5),
  TAN_CV(IEM_crx,5),
  TAN_CV(IEM_diabetes,5),
  TAN_CV(IEM_flare,5),
  TAN_CV(IEM_german,5),
  TAN_CV(IEM_glass,5),
  TAN_CV(IEM_glass2,5),
  TAN_CV(IEM_heart,5),
  TAN_CV(IEM_hepatitis,5),
  TAN_CV(IEM_iris,5),
  TAN_ST(IEM_letter, 5000),
  TAN_CV(IEM_lympograph,5),
  TAN_CV(IEM_mofn,5),
  TAN_ST(IEM_satimage,2000),
  TAN_ST(IEM_segment,770),
  TAN_ST(IEM_shuttle, 1934),
  TAN_CV(IEM_soybean,5),
  TAN_CV(IEM_vehicle,5),
  TAN_CV(IEM_vote,5),
  TAN_ST(IEM_wave,4700)
  )
  result[,exp_num] = acc
}
result[,11] = apply(result[ ,1:10], 1, mean)
result[,12] = apply(result[ ,1:10], 1, sd)
result[,11] = result[,11]*100
write.csv(result, file = "TAN_result.csv")
